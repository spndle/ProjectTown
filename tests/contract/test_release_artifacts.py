...
def test_release_ci_keeps_quality_and_coverage_gates() -> None:
    ...
    removed_diagnostics = (
        "diagnose" + "_windows_acl",
        "PROJECTTOWN" + "_ACL_TRACE",
        "WINDOWS_ACL" + "_TRACE_MARKERS",
        "trace" + "=True",
    )
    assert all(value not in workflow for value in removed_diagnostics)
    assert not (ROOT / "scripts" / ("diagnose" + "_windows_acl.py")).exists()
    assert not (ROOT / "tests" / ("test_" + "diagnose" + "_windows_acl.py")).exists()
